...
// Row shape returned by GET /api/oprolemember and /api/oprolemember/view/{id}
// -- matches the OpRoleMemberView class generated for the dotnet/rust servers.
export interface OpRoleMemberView {
  id: number;
  principal_id: number;
  op_role_id: number;
  is_active: number;
...
const DOMAIN_OBJECT_NAME = "OpRoleMember";
...
async function fetchList(api: ApiClient): Promise<OpRoleMemberView[]> {
  return api.get<OpRoleMemberView[]>("/api/oprolemember");
}

export default function ListOpRoleMember() {
  const api = useApiClient();
  const navigate = useNavigate();
  const location = useLocation();

  const [list, setList] = useState<OpRoleMemberView[] | null>(null);
  ...
  const listRef = useRef<OpRoleMemberView[] | null>(null);
  useEffect(() => {
  ...
  const reloadList = useCallback(async () => {
    try {
      ...
      console.error("ListOpRoleMember: error reloading list", err);
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    fetchList(api)
      ...
      .catch((err) => console.error("ListOpRoleMember: error loading list", err));
  ...
  useEffect(() => {
    ...
    source.addEventListener("PropertyUpdated", async (event: MessageEvent) => {
      try {
        ...
        if (index >= 0) {
          const updated = await api.get<OpRoleMemberView>(`/api/oprolemember/view/${message.InstanceId}`);
          setList((current) => {
        ...
        console.error("ListOpRoleMember: error handling notification", err);
      }
    });
  ...
  function onAdd() {
    ...
    navigate(`/edit-oprolemember?returnUrl=${returnUrl}`);
  }

  function onEdit(item: OpRoleMemberView) {
    const returnUrl = encodeURIComponent(location.pathname);
    navigate(`/edit-oprolemember/${item.id}?returnUrl=${returnUrl}`);
  }

  return (
    <>
      <h1>Operation Role Members</h1>
      <p>This page demonstrates fetching Operation Role Members data from the server.</p>
